// Stresses the write-overlay's *enumeration splice* through node's fs directory
// APIs (readdirSync -> FindFirstFile/FindNextFile). Enumeration is the overlay's
// hardest area: a single listing of one directory must merge the REAL on-disk
// entries (returned by the underlying FindFirstFile) with the OVERLAY-ONLY
// entries (created this process into the backing store) - without duplicates,
// reflecting the removal of an overlay-created entry - all while the real
// execroot stays byte-for-byte unchanged.
//
// NOTE: we only ever create/delete OVERLAY entries here. The overlay is
// backing-store-as-truth with NO whiteout markers (see
// docs/design/detours-write-overlay-vfs.md 6.3.1), so a REAL visible in-cone
// file is immutable: deleting or renaming it is denied in permissive mode. The
// real entries therefore always enumerate through the passthrough unchanged;
// that immutability is what makes the merge (not a mutation) the thing to test.
//
// The test pre-seeds <execroot>/mix on the real disk with realA.txt/realB.txt.
//
// Run as: node enum_ops.js <execroot>
// Emits a space-separated set of markers the test asserts on:
//   LIST1=<entries>     baseline listing of the seeded (real) dir
//   LIST2=<entries>     listing after splicing overlay entries + one deletion
//   GLOBOV=<entries>    merged listing filtered to the "ov" prefix
//   GLOBREAL=<entries>  merged listing filtered to the "real" prefix
//   READOV=<content>    read-back of an overlay-only file
//   READREAL=<content>  passthrough read-back of a real file
"use strict";
const fs = require("fs");
const p = require("path");

const ws = process.argv[2];
const d = p.join(ws, "mix"); // pre-seeded on the real disk

const list = () => fs.readdirSync(d).sort().join(",");

// baseline: the seeded real entries enumerate through the passthrough
const list1 = list();

// splice overlay-only entries (files + a subdir) into the SAME directory that
// already holds the real entries
fs.writeFileSync(p.join(d, "ovX.txt"), "OVX");
fs.writeFileSync(p.join(d, "ovY.txt"), "OVY");
fs.mkdirSync(p.join(d, "ovsub"));
fs.writeFileSync(p.join(d, "ovsub", "inner.txt"), "INNER");

// delete one OVERLAY-created entry: it must drop out of the merged view while
// the real entries (and the other overlay entries) remain
fs.rmSync(p.join(d, "ovY.txt"));

// merged view: realA/realB + ovX.txt + ovsub, but not ovY.txt
const list2 = list();

// prefix filters over the merged set, both halves
const globOv = fs.readdirSync(d).filter((n) => n.startsWith("ov")).sort().join(",");
const globReal = fs.readdirSync(d).filter((n) => n.startsWith("real")).sort().join(",");

// read-back: an overlay-only file and a real (passthrough) file
const readOv = fs.readFileSync(p.join(d, "ovX.txt"), "utf8");
const readReal = fs.readFileSync(p.join(d, "realA.txt"), "utf8");

process.stdout.write(
    "LIST1=" + list1 +
    " LIST2=" + list2 +
    " GLOBOV=" + globOv +
    " GLOBREAL=" + globReal +
    " READOV=" + readOv +
    " READREAL=" + readReal
);
