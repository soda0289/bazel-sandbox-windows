...
* **Bazel integration harnesses** (`tests/integration/sandbox-strategy-smoke.ps1`
  and `smoke.ps1`) drive the sandbox through a **real Bazel build** to validate the
  **Bazel <-> sandbox integration layer** (that `windows-sandbox` passes
  `--filter-inputs --write-overlay`, grants declared inputs, hides undeclared ones).
